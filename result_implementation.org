...
* Questionarie module
  ...
  question-id which is simply logged to the output, a text description
  for prompting the user, and finally a mapping between keyboard key
  ...
  matches a response alternative, the data is then stored in
  memory. This process is repeated untill each configured question has
  been performed.  has been processed. After the questinarie task is
  completed the results are saved to disk in the comma separated value
  format.
