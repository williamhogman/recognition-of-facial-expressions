The resulting artifact is comprised of a number of components. These
are, the METT module, the questionnaire module, run-time
configuration, deployment scripts and text assets. These components
are replaceable with other components on an as-needed basis. A
graphical representation of the module dependencies is presented in
figure \ref{fig:modules}. In the coming sections the developed
artifact will be discussed in detail.

The artifact developed does to a large extent not use the object
oriented paradigm of software development, instead, a design based on
modules containing functions were used. Where possible unnessary
modifications of state were avoided. Indeed, outside of functions
interacting concerned with screen rendering, input management and
input and output, state is not modified. Additionally, inputs were
read once and not mutated after creation, outputs only written once
and screen rendering produced no alternations of state as visible to
the main artifact. After configuration, the only state read is user
...
#+BEGIN_LATEX
  \begin{figure}[htpb]
    ...
    \end{tikzpicture}
    \caption{Graph of dependencies in the software artifact}
  \end{figure}
#+END_LATEX
