...
#+BEGIN_LATEX
  \begin{tikzpicture}[
    ...
    \node (experiment) [] {Experiment};
    \node (mett) [below =1cm of experiment] {METT};
    \node (questionnarie) [] {Questionnarie};
    \node (config) [] {Config};
    \node (util) [] {Utilities};
    \node (ui) [] {UI};

    \path[draw,thick]
    (experiment) edge node {} (mett)
    (experiment) edge node {} (questionnarie)
    (experiment) edge node {} (config)
    (questionnarie) edge node {} (ui)
%    (mett) edge node {} (ui)
    (ui) edge node {} (util);
